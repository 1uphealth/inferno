...
require_relative './profile_definitions/us_core_smokingstatus_definitions'
...
module Inferno
  module Sequence
    class USCore310SmokingstatusSequence < SequenceBase
      ...
      description 'Verify support for the server capabilities required by the US Core Smoking Status Observation Profile.'

      details %(
        # Background

        The US Core #{title} sequence verifies that the system under test is able to provide correct responses
        for Observation queries.  These queries must contain resources conforming to US Core Smoking Status Observation Profile as specified
      ...
      test 'Server returns Provenance resources from Observation search by patient + code + _revIncludes: Provenance:target' do
        ...
        save_delayed_sequence_references(provenance_results, USCore310SmokingstatusSequenceDefinitions::DELAYED_REFERENCES)
      ...
      test 'All must support elements are provided in the Observation resources returned.' do
        ...
        skip_if_not_found(resource_type: 'Observation', delayed: false)
        must_supports = USCore310SmokingstatusSequenceDefinitions::MUST_SUPPORTS
