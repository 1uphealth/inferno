...
require_relative './profile_definitions/bodyweight_definitions'
...
module Inferno
  module Sequence
    class USCore310BodyweightSequence < SequenceBase
      ...
      description 'Verify support for the server capabilities required by the Observation Body Weight Profile.'

      details %(
        # Background

        The US Core #{title} sequence verifies that the system under test is able to provide correct responses
        for Observation queries.  These queries must contain resources conforming to Observation Body Weight Profile as specified
        ...
        ## Profile Validation
        Each resource returned from the first search is expected to conform to the [Observation Body Weight Profile](http://hl7.org/fhir/StructureDefinition/bodyweight).
      ...
      test 'Server returns Provenance resources from Observation search by patient + code + _revIncludes: Provenance:target' do
        ...
        save_delayed_sequence_references(provenance_results, USCore310BodyweightSequenceDefinitions::DELAYED_REFERENCES)
      ...
      test :validate_resources do
        ...
        bindings = USCore310BodyweightSequenceDefinitions::BINDINGS
      ...
      test 'All must support elements are provided in the Observation resources returned.' do
        ...
        skip_if_not_found(resource_type: 'Observation', delayed: false)
        must_supports = USCore310BodyweightSequenceDefinitions::MUST_SUPPORTS
