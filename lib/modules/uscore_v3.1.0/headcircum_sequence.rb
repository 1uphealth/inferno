...
module Inferno
  module Sequence
    class USCore310HeadcircumSequence < SequenceBase
      ...
      description 'Verify support for the server capabilities required by the Observation Head Circumference Profile.'

      details %(
        # Background

        The US Core #{title} sequence verifies that the system under test is able to provide correct responses
        for Observation queries.  These queries must contain resources conforming to Observation Head Circumference Profile as specified
        ...
        ## Profile Validation
        Each resource returned from the first search is expected to conform to the [Observation Head Circumference Profile](http://hl7.org/fhir/StructureDefinition/headcircum).
      ...
      test :validate_resources do
        metadata do
          id '10'
          name 'Observation resources returned from previous search conform to the Observation Head Circumference Profile.'
          link 'http://hl7.org/fhir/StructureDefinition/headcircum'
          description %(

            This test verifies resources returned from the first search conform to the [US Core Observation Profile](http://hl7.org/fhir/StructureDefinition/headcircum).
        ...
        bindings = USCore310HeadcircumSequenceDefinitions::BINDINGS
        ...
        bindings.select { |binding_def| binding_def[:strength] == 'extensible' }.each do |binding_def|
          begin
            ...
            if invalid_bindings.present?
              invalid_bindings = resources_with_invalid_binding(binding_def.except(:system), @observation_ary&.values&.flatten)
