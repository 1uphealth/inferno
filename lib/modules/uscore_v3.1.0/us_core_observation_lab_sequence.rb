...
module Inferno
  module Sequence
    class USCore310ObservationLabSequence < SequenceBase
      ...
      description 'Verify support for the server capabilities required by the US Core Laboratory Result Observation Profile.'

      details %(
        # Background

        The US Core #{title} sequence verifies that the system under test is able to provide correct responses
        for Observation queries.  These queries must contain resources conforming to US Core Laboratory Result Observation Profile as specified
