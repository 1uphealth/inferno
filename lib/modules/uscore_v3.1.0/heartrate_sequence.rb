...
module Inferno
  module Sequence
    class USCore310HeartrateSequence < SequenceBase
      ...
      description 'Verify support for the server capabilities required by the Observation Heart Rate Profile.'

      details %(
        # Background

        The US Core #{title} sequence verifies that the system under test is able to provide correct responses
        for Observation queries.  These queries must contain resources conforming to Observation Heart Rate Profile as specified
        ...
        ## Profile Validation
        Each resource returned from the first search is expected to conform to the [Observation Heart Rate Profile](http://hl7.org/fhir/StructureDefinition/heartrate).
      ...
      test :validate_resources do
        ...
        bindings = USCore310HeartrateSequenceDefinitions::BINDINGS
